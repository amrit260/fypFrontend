...
if(!props.tours) return''
 ...
 let tours;
    ...
    const category = new URLSearchParams(queryString).get('category')
       tours = props.tours.data.filter(item=>category.includes(item.category.toString()) || members===item.members || duration === item.duration)
       console.log(members)
    }
  ...
  // }
  const tourCards = tours.data.map((tour,index)=>{
      if(index<props.tourCount){
          ...
          <img src={`${URL}/img/tours/${tour.images[0]}`} alt="" className="card-img-top" />
